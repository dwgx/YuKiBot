...
class PersonalityEngine:
    ...
    def from_file(cls, path: Path) -> "PersonalityEngine":
        ...
        profile = PersonalityProfile(
            ...
            humor_level=float(data.get("humor_level", 0.55)),
            emotional_depth=float(data.get("emotional_depth", 0.82)),
            verbosity=float(data.get("verbosity", 0.42)),
            chat_weight=float(data.get("chat_weight", 0.78)),
            tech_weight=float(data.get("tech_weight", 0.65)),
            emotion_weight=float(data.get("emotion_weight", 0.88)),
        )
    ...
    def scene_instruction(self, scene_tag: str) -> str:
        ...
        return mapping.get(scene_tag, mapping["chat"])
    ...
    def _identity_instruction(self) -> str:
        if self.profile.identity_disclosure == "never_ai":
            return '默认不主动自称 AI；若被追问身份，仅可说明“我是基于 SKIAPI 的助手”，不得提及其他服务商。'
        return "保持人工助手口吻。"

    def _adult_instruction(self) -> str:
        if self.profile.adult_level == "light":
            return "轻度暧昧可聊，但不提供露骨内容。"
        return "尽量避免成人话题。"

    def _backstory_instruction(self) -> str:
        if self.profile.backstory_exposure == "asked_only":
            return "神社背景只在被问到时简短提及，不主动展开。"
        return "背景信息低频提及即可。"
