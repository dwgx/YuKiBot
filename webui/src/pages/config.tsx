...
const SECTIONS: SectionDef[] = [
  { key: "control", label: "总控面板", fields: [
    { path: "control.heuristic_rules_enable", label: "启用关键词/正则规则", type: "switch" },
  ...
  { key: "bot", label: "Bot 设置", fields: [
    ...
    { path: "bot.voice_send_try_full_first", label: "优先尝试完整语音", type: "switch" },
    { path: "bot.voice_send_split_enable", label: "启用语音分段", type: "switch" },
  ...
  { key: "search", label: "搜索设置", fields: [
    ...
    { path: "search.tool_interface.browser_enable", label: "启用浏览器工具", type: "switch" },
    { path: "search.tool_interface.local_enable", label: "启用本地文件工具", type: "switch" },
    { path: "search.tool_interface.github_enable", label: "启用 GitHub 工具", type: "switch" },
  ...
  { key: "routing", label: "路由设置", fields: [
    { path: "routing.mode", label: "路由模式", type: "select", options: [
      ...
      { value: "keyword_first", label: "keyword_first(关键词优先)" },
      { value: "hybrid", label: "hybrid(混合模式)" }
    ]},
    { path: "routing.trust_ai_fully", label: "完全信任AI判定", type: "switch" },
    { path: "routing.enable_keyword_heuristics", label: "启用关键词启发式", type: "switch" },
...
function parseNumberInput(raw: string, current: unknown, field: FieldDef): number { const text = raw.trim(); if (text === "" || text === "-" || text === "." || text === "-.") return typeof current === "number" ? current : 0; const n = Number.parseFloat(text); if (!Number.isFinite(n)) return typeof current === "number" ? current : 0; let next = n; if (typeof field.min === "number") next = Math.max(field.min, next); if (typeof field.max === "number") next = Math.min(field.max, next); return next; }
...
export default function ConfigPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await api.updateConfig(withDefaults(payload));
  ...
  const renderField = (field: FieldDef) => {
    ...
    const control = (() => {
      ...
      if (field.type === "number") {
        return <Input label={field.label} labelPlacement="outside" type="number" value={String(val ?? "")} min={field.min} max={field.max} step={field.step} onValueChange={(v) => updateField(field.path, parseNumberInput(v, val, field))} classNames={INPUT_CLASSES} />;
      }
      if (field.type === "list") {
